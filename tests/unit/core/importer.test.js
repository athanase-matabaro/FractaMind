...
jest.mock('../../../src/core/federation', () => ({
  initFederation: jest.fn().mockResolvedValue(undefined),
  addProjectIndex: jest.fn().mockResolvedValue({ indexed: 0, quantParams: {} }),
}));
...
describe('importer', () => {
  ...
  describe('handleSeedSubmit (integration)', () => {
    beforeEach(() => {
      ...
      // Mock checkAIAvailability
      chromeAI.checkAIAvailability.mockReturnValue({
        available: {
          summarizer: true,
          embeddings: true,
          writer: true,
    ...
    it('should handle errors gracefully', async () => {
      ...
      // Watchdog catches the error and returns fallback, so we expect a fallback-related error
      // or successful completion with mock data
